...
export class NumberInputComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.amount && this.amount.trim() !== '') {
      ...
      let newAmount = +this.amount * Math.pow(10, this.precision)
  ...
  get formattedAmount(): string {
    ...
    //let formatedAmount = this.currencyPipe.transform(+this.amount / Math.pow(10, this.precision), 'USD', true, '1.0-4');

    let formatedAmount = this.currencyPipe.transform(+this.amount / Math.pow(10, this.precision), this.currencyType + ' ', true, '1.0-4');
